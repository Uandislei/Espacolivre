...
export default function EditarAnuncio() {
  ...
  const [form, setForm] = useState({ title:"", description:"", market_category:"", city:"", state:"", price_label:"", image_url:"", whatsapp:"" });
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data, error } = await supabase.from("spaces").select("title,description,market_category,city,state,price_label,image_url,whatsapp").eq("id", id).eq("owner_id", user.id).maybeSingle();
  ...
  return <main className="page narrow">
    ...
    <form className="form" onSubmit={submit}>
      ...
      <div className="form-grid"><label>Preço exibido<input value={form.price_label} onChange={e=>set("price_label",e.target.value)} /></label><label>Foto principal (URL)<input value={form.image_url} onChange={e=>set("image_url",e.target.value)} placeholder="https://..." /></label><label>WhatsApp<input value={form.whatsapp} onChange={e=>set("whatsapp",e.target.value)} required /></label></div>
